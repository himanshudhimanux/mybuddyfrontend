...
import { fetchSessions, fetchLocations, fetchCourses, setSessionYear, setLocationId, setCourseId, setBatchName, createBatch } from '../../redux/features/batch/batchSlice';
import toast from 'react-hot-toast';

const AddBatch = () => {
  const dispatch = useDispatch();
  const { sessions, locations, courses, sessionYear, locationId, courseId, batchName, loading } = useSelector(state => state.batches);
  ...
  const handleSubmit = (e) => {
    ...
    if (batchName && sessionYear && locationId && courseId) {
      const batchData = {
        batchName,
        sessionYear,
        locationId,
        ...
        <div className="mb-4">
          ...
          >
            {sessions.map(session => (
              <option key={session._id} value={session._id}>{session.yearName}</option>
            ))}
          </select>
        </div>
